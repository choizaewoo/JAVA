...
public class JAVA14 {
	...
	public static void main(String[] args) {
		...
		System.out.printf("총 수수료: %s원 => 미화: %s달러, 달러당 수수료: %f원\n",
			k42_df.format(k42_i_totalCom), //DecimalFormat형으로 가져온다.
			k42_df.format(k42_usd), //DecimalFormat형으로 가져온다.
			k42_ComPerOne); //DecimalFormat형으로 가져온다.

		
		k42_remain = (int) (k42_MyWon - k42_usd * k42_MoneyEx - k42_i_totalCom);
		System.out.printf("총 한화 환전 금액: %s원 => 미화: %s달러 수수료징구: %s원 잔돈: %s원\n",
				k42_df.format(k42_MyWon), //DecimalFormat형으로 가져온다.
				k42_df.format(k42_usd),  //DecimalFormat형으로 가져온다.
				k42_df.format(k42_i_totalCom),  //DecimalFormat형으로 가져온다.
				k42_df.format(k42_remain)); //DecimalFormat형으로 가져온다.
	}
}
